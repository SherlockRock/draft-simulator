...
import {
  getCurrentTurn, applyMove, isTerminal, remainingPool,
} from "./draft-state.js";
...
function minimax(
  ...
  const candidates = ordered.slice(0, Math.max(ctx.config.branchWidth, ordered.length));
  const children: Array<{ championId: string; result: SearchResult }> = [];
  ...
  if (isUserTurn) {
    ...
    for (const champId of candidates) {
      ...
      children.push({ championId: champId, result });
  ...
  return {
    ...
    node: {
      ...
      slot: null,
      userInjected: false,
      children: topChildren.map(({ championId, result }) => ({
        ...result.node,
        championId,
        side: turn.side,
        slot: state.turnIndex,
        userInjected: false,
      })),
    },
  };
